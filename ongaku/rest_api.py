...
class Rest:
    ...
    async def fetch_info(self) -> models.Info:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(self._link._standard_uri+"/info", headers=self._link._headers) as response:
    ...
    async def search(self, platform: models.PlatformType, query: str) -> t.Optional[list[models.Track]]:
        async with aiohttp.ClientSession() as session:
            if platform == models.PlatformType.YOUTUBE:
                ...
                params = {
                    "identifier": f"ytmsearch:\"{query}\""
                }
            else:
                params = {
                    "identifier": f"scsearch:\"{query}\""
                }
            
            async with session.get(self._link._standard_uri+"/loadtracks", headers=self._link._headers, params=params) as response:
                if response.status >= 400:
                    raise error.ResponseException(response.status)
    ...
    async def load_track(self, track: models.Track, guild_id: int) -> None:
        async with aiohttp.ClientSession() as session:
            params = {
                "identifier": track.encoded
            }
            async with session.get(self._link._standard_uri, headers=self._link._headers, params=params) as response:
                if response.status >= 400:
                    raise error.ResponseException(response.status)
